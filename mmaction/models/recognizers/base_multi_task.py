...
class BaseRecognizer_MultiTask(BaseModel, metaclass=ABCMeta):
    ...
    def __init__(self,
                 ...
                 cls_head_2: OptConfigType = None,
                 cls_head_3: OptConfigType = None,
        ...
        super(BaseRecognizer_MultiTask,
        ...
        if cls_head_2 is not None:
            self.cls_head_2 = MODELS.build(cls_head_2)

        if cls_head_3 is not None:
            self.cls_head_3 = MODELS.build(cls_head_3)
    ...
    def loss(self, inputs: torch.Tensor, data_samples: SampleList,
        ...
        if self.cls_head_2 is not None:
            # labels = [x.gt_label for x in data_samples]
            # labels = torch.stack(labels).to(cls_scores.device)
            # labels = labels.squeeze()
            loss_cls_2 = self.cls_head_2.loss(feats, data_samples, **loss_kwargs)
        else:
            loss_cls_2 = {}
        
        if self.cls_head_3 is not None:
            loss_cls_3 = self.cls_head_3.loss(feats, data_samples, **loss_kwargs)
        else:
            loss_cls_3 = {}      
        
        losses = merge_dict(loss_cls, loss_cls_2, loss_cls_3, loss_aux)
        return losses
    ...
    def predict(self, inputs: torch.Tensor, data_samples: SampleList,
        ...
        predictions = self.cls_head.predict(feats, data_samples,
                                            **predict_kwargs) # task3
        predictions_2 = self.cls_head_2.predict(feats, data_samples, **predict_kwargs) # task2
        predictions_3 = self.cls_head_3.predict(feats, data_samples, **predict_kwargs) # task1
        # import pdb; pdb.set_trace()
        return predictions, predictions_2, predictions_3
