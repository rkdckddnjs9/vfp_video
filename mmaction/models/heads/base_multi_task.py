...
class BaseHead_MultiTask(BaseModule, metaclass=ABCMeta):
    """Base class for head.
    ...
    def __init__(self,
        ...
        super(BaseHead_MultiTask, self).__init__(init_cfg=init_cfg)
    ...
    def loss_by_feat(self, cls_scores: torch.Tensor,
        ...
        if self.num_classes == 7: #task3
            pass
        elif self.num_classes == 3: #task2
            # LP, LM, LL -> 1  , RP, RM, RL -> 2
            L_mask = (labels==1) + (labels==2) + (labels==3)
            R_mask = (labels==4) + (labels==5) + (labels==6)
            labels[L_mask[:, 0]] = 1
            labels[R_mask[:, 0]] = 2         
        elif self.num_classes == 2: #task1
            vfp_mask = (labels!=0)
            labels[vfp_mask[:, 0]] = 1
        
        labels = labels.squeeze().to(cls_scores.device)
        ...
        if cls_scores.size() != labels.size():
            ...
            for k, a in zip(self.topk, top_k_acc):
                if self.num_classes == 3: #task2
                    losses[f'task2_top{k}_acc'] = torch.tensor(
                        a, device=cls_scores.device)
                elif self.num_classes == 2: #task1
                    losses[f'task1_top{k}_acc'] = torch.tensor(
                        a, device=cls_scores.device)
                else:
                    losses[f'task3_top{k}_acc'] = torch.tensor(
                        a, device=cls_scores.device)
        ...
        if isinstance(loss_cls, dict):
            ...
            if self.num_classes == 3: #task2  
                losses['task2_loss_cls'] = loss_cls
            elif self.num_classes == 2: #task1  
                losses['task1_loss_cls'] = loss_cls
            else:  
                losses['task3_loss_cls'] = loss_cls
        return losses
    ...
    def predict_by_feat(self, cls_scores: torch.Tensor,
        ...
        for data_sample, score, pred_label in zip(data_samples_, cls_scores,
                                                  pred_labels):
            if self.num_classes == 7: #task3
                pass
            elif self.num_classes == 3: #task2
                # LP, LM, LL -> 1  , RP, RM, RL -> 2
                label_map = {0:0, 1:1, 2:1, 3:1, 4:2, 5:2, 6:2}
                gt_label = torch.tensor([label_map[data_sample.gt_label.item()]])
                data_sample.gt_label = gt_label
                data_sample.set_field(name='num_classes', value=self.num_classes, field_type='metainfo')  
            elif self.num_classes == 2: #task1
                # LP, LM, LL, RP, RM, RL -> 1
                label_map = {0:0, 1:1, 2:1, 3:1, 4:1, 5:1, 6:1}
                gt_label = torch.tensor([label_map[data_sample.gt_label.item()]])
                data_sample.gt_label = gt_label
                data_sample.set_field(name='num_classes', value=self.num_classes, field_type='metainfo')  
